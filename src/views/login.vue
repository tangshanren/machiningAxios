<template>
  <div class="login-container" :loading="loading">
    <el-form
      :model="myForm"
      :rules="myRule"
      status-icon
    ...
    >
      ...
      <el-form-item prop="password">
        <el-input type="password" v-model="myForm.password" auto-complete="off" placeholder="密码" />
      </el-form-item>
          ...
          setTimeout(() => {
            if (
              ...
              localStorage.setItem('token','555')
                // 将登录名使用vuex传递到Home页面
              this.$store.commit('handleUserName',this.admin);
            } else {
              this.logining = false;
